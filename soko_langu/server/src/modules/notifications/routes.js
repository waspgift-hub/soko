...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    const result = await notificationService.listNotifications(req.firebaseUid, { page, limit });
...
router.get('/unread-count', authenticate, async (req, res) => {
  try {
    const count = await notificationService.unreadCount(req.firebaseUid);
...
router.post('/:id/read', authenticate, async (req, res) => {
  try {
    const ok = await notificationService.markRead(req.firebaseUid, req.params.id);
...
router.post('/read-all', authenticate, async (req, res) => {
  try {
    const count = await notificationService.markAllRead(req.firebaseUid);
...
router.delete('/:id', authenticate, async (req, res) => {
  try {
    const ok = await notificationService.deleteNotification(req.firebaseUid, req.params.id);
    if (!ok) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Notification not found' } });
...
router.delete('/', authenticate, async (req, res) => {
  try {
    const count = await notificationService.deleteAll(req.firebaseUid);

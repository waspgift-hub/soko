...
class _EarningsDashboardState extends State<EarningsDashboard> {
  ...
  Widget _buildBalanceCard(double balance, double totalEarnings) {
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            Text('${nf.format(balance)} TZS',
                style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold)),
            Text('Available Balance',
                style: TextStyle(color: Colors.grey[600], fontSize: 14)),
            const SizedBox(height: 8),
            Text('Total Earned: ${nf.format(totalEarnings)} TZS',
  ...
  Widget _buildStatsRow() {
    ...
    return Row(
      children: [
        Expanded(
          child: _statTile('Today\'s Views', nf.format(_adViewsToday), Icons.visibility),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: _statTile('This Month', nf.format(_adViewsMonth), Icons.visibility),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: _statTile('Rev Share', '40%', Icons.percent),
        ),
      ],
    );
  }
          ...
          children: docs.map((doc) {
            ...
            return Card(
              child: ListTile(
                ...
                trailing: Text('+${nf.format(amount)} TZS',

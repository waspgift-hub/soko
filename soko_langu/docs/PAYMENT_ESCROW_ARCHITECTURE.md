...
- **Phase 2 — Refund service:** v2 `refunds` module with `Refund` state machine (pending → processing → completed), escrow release to buyer + `REFUND_PROCESSED`, partial refund support, notifications, admin UI, tests.
- **Phase 3 — Dispute completion:** PARTIAL split amounts, escrow split to seller/buyer, `DisputeEvidence` upload flow, SLA escalation.
- **Phase 4 — Migration of buyers (web shop):** wire shop checkout to v2 order/payment endpoints; keep legacy read-only compat; data MIGRATION for orders in escrow (Firestore → Postgres, preserving state and `paidAt`).
- **Phase 5 — Migration of sellers (Flutter):** seller flows to v2 wallet/withdrawal/payout; set `commission` parity; backfill wallets from historical sales; retire autoPayout.
- **Phase 6 — Parity & decommission:** reconcile ledger vs Firestore totals; align commission model (buyer-top-up vs deduct-from-seller); remove money mutations from legacy-compat; kill legacy money endpoints; keep endpoints read-only.

...
- **Phase 6 — Parity & decommission — step 2 (web-shop parity) shipped:** the `/shop` SPA is now feature-parity with the Flutter storefront, plus guest checkout and an in-browser seller dashboard. Product cards match `product_card.dart` (featured gradient flag, KYC verified seal, red discount pill, `· new` tag, ratings `★ (n)` + sold count, sold-out overlay); product detail gained share (WhatsApp + copy link), a star-rated review section (write/read/recompute product `rating`/`reviewCount` client-side — rules permit only that diff) and a wishlist (localStorage `sv_shop_wish` + header badge + `#/wishlist` page). Checkout no longer blocks guests: `POST /api/orders/create` self-resolves a synthetic `guest:<phone>` buyer (Postgres `users` row), returns `buyerId` (Postgres id) which the SPA persists as `sv_shop_guest` (`{buyerId, phone, name}`) and re-supplies on every follow-up (`/api/orders/guest/list`, `/api/orders/:id/status`, payment link) — identity is pinned to phone+name via `resolveShopBuyer` (403 `FORBIDDEN` on mismatch; the 401 `AUTH_REQUIRED` guest gate is gone). Mirror `orders`/`transactions` docs are presentation-only (`sourceV2:true`, `escrowStatus:'none'`); `reconcile-money.js` now skips `sourceV2` docs (their truth is Postgres) and `finance.inspectionAutoRelease` still skips `legacyFirestoreId`. Seller dashboard (`#/seller`): overview stats, product CRUD (create mirrors the Flutter product-service shape; edits limited to non-boost/feature fields the rules allow owners to write; publish/unpublish `isActive`; delete; wholesale rows), store orders (Firestore read now permitted by rules), wallet balance + legacy `POST /api/payouts/seller/withdraw` (min 10 000 TZS). 137/137 unit tests green incl. `legacy-shop.test.js` (guest identity, mirror, list). Remaining (unchanged): retire `autoPayout`, make legacy money endpoints read-only, kill legacy money mutations, seller-app (Flutter) wallet on v2.
- **Phase 6 — Parity & decommission — step 3 (legacy endpoint parity + SPA wiring) shipped:** the remaining 25 legacy endpoints absent from v2 were ported verbatim into `server/src/modules/legacy-compat/feature-compat.js` and mounted at their original `/api/*` paths — seller analytics/finance (`balance`, `products`, `payouts`, `withdrawals`), marketplace discovery, notifications (list/mark-read/unread-count/preferences), SMS send, OTP, KYC check, flash-sale create/scan/notify, admin stats/metrics/analytics/users/finance-summary/audit-logs, fraud/reports/statements, escalation, referral, moderation, upload, escalation, plus `POST /api/admin/broadcast-notification` (all-users query + OneSignal bulk + in-app batch 500, legacy response shape). Money/parity safeguards: `BigInt` serializes as numbers everywhere (`BigInt.prototype.toJSON` in `src/app.js`), and the funded-escrow dispatch chain runs the canonical machine (`IN_ESCROW → ready_to_dispatch → dispatched → in_transit → out_for_delivery → delivered → inspection_period → OTP_PENDING`) with `statusChangedBy/statusChangedAt`, mirroring to the legacy Firestore `orders`/`transactions` docs. The `/shop` SPA is wired to all of it via `server/shop/parity.js` (loaded AFTER `app.js`; ACTIONS merged on `DOMContentLoaded`; `route()` deferred so parity renderers exist): `#/notifications` + `#/notifprefs`, `#/chats` + `#/chat/:uid`, `#/flash`, seller tabs analytics/flash/kyc/boost, buyer order-area (`parityBuyerOrderArea` via `/api/v1/orders/:id`), product extras (`recordRecent` + recents + chat room), auth alt-buttons, all data.js `t()` keys filled (sw/en, 0 missing). **Found-and-fixed (E2E):** `POST /api/v1/orders/:orderId/cancel` returned 500 for EVERY caller — it passed `actor:'user'` to the state machine whose CANCELLED actor set is buyer/seller/admin, and never checked ownership. Fixed in `order-service.cancelOrder` (derive role from order ownership: buyer = `order.buyerId`, seller = their `SellerProfile.id`, else admin; otherwise 403 FORBIDDEN); funded states still refuse with `CANNOT_CANCEL_IN_CURRENT_STATE` (400). Live verification, no real payment: full suite **188/188 green**, live contract suite **51/51 green**, and `server/scripts/live-buyer-flow.mjs` (fresh Firebase buyer → `/api/orders/create` → status mirror `pending` → v2 read-back `awaiting_escrow_payment` → cancel → `cancelled` on both surfaces → cross-user isolation 401) **11/11**. ClickPesa USSD PIN was deliberately NOT exercised — payment verification stands on the automated suite + provider webhook tests, not a live charge.

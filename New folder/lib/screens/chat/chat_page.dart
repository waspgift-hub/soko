...
class _ChatPageState extends State<ChatPage> {
          ...
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  status == 'declined'
                      ? context.tr('call_declined')
                      : status == 'missed'
                          ? context.tr('call_missed')
                          : context.tr('call_cancelled'),
                ),
              ...
              PopupMenuItem(
                ...
                child: Row(
                  children: [
                    ...
                    Text(context.tr('voice_call'), style: const TextStyle(fontSize: 14)),
                  ],
                ),
              ),
              PopupMenuItem(
                ...
                child: Row(
                  children: [
                    Icon(Icons.videocam, color: cs.primary, size: 20),
                    const SizedBox(width: 12),
                    Text(context.tr('video_call'), style: const TextStyle(fontSize: 14)),
                  ],
                ),
              ),
            ],
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  const Spacer(),
                  GestureDetector(onTap: _cancelEdit, child: Icon(Icons.close, size: 18, color: cs.onSurface.withOpacity(0.6))),
                ],
              ),
            ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(_replyToContent ?? '', maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 12, color: cs.onSurface.withOpacity(0.6))),
                      ],
                    ),
                  ),
                  GestureDetector(
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              if (isDeleted)
                Text(context.tr('deleted_for_everyone'), style: TextStyle(fontSize: 15, fontStyle: FontStyle.italic, color: isMe ? Colors.white70 : cs.onSurface.withOpacity(0.6)))
              else if (isVoice)
                Container(
                  padding: const EdgeInsets.all(8),
                  decoration: BoxDecoration(color: isMe ? Colors.white.withOpacity(0.2) : cs.primary.withOpacity(0.1), borderRadius: BorderRadius.circular(8)),
              ...
              Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Text('${message.timestamp.hour}:${message.timestamp.minute.toString().padLeft(2, '0')}', style: TextStyle(fontSize: 11, color: isMe ? Colors.white70 : cs.onSurface.withOpacity(0.6))),
                  if (message.isEdited && !isDeleted) Text(context.tr('edited'), style: TextStyle(fontSize: 11, fontStyle: FontStyle.italic, color: isMe ? Colors.white60 : cs.onSurface.withOpacity(0.4))),
                  if (isMe) ...[
                    const SizedBox(width: 4),
                    Icon(
                      message.isRead ? Icons.done_all : message.isDelivered ? Icons.done_all : Icons.done,
                      size: 14,
                      color: message.isRead ? const Color(0xFF8ECAE6) : message.isDelivered ? (isMe ? Colors.white38 : cs.onSurface.withOpacity(0.4)) : Colors.white60,
                    ),
                  ],
                ],
              ),
  ...
  Widget _buildReplyPreview(Message message, bool isMe, ColorScheme cs) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: isMe ? Colors.white.withOpacity(0.15) : cs.primary.withOpacity(0.1),
        borderRadius: BorderRadius.circular(8),
        border: Border(left: BorderSide(color: isMe ? Colors.white.withOpacity(0.4) : cs.primary, width: 3)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(message.replyToSender ?? '', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isMe ? Colors.white.withOpacity(0.8) : cs.primary)),
          const SizedBox(height: 2),
          Text(message.replyToContent!, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 12, color: isMe ? Colors.white70 : cs.onSurface.withOpacity(0.6))),
        ],
      ),
    );
  }
        ...
        children: reactions.entries.map((entry) {
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: isMe ? Colors.white.withOpacity(0.2) : Colors.grey.shade200, borderRadius: BorderRadius.circular(12)),
